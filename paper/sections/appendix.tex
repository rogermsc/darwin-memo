...
\begin{tabular}{lcccc}
\toprule
 & benign & silence & harmful safe & pop \\
\midrule
...
\paragraph{Verdict.}
On this protocol the pre-registered claim that outcome-based
...
Replacing measured settlement with keep-or-cull verdicts from a local model
...
magnitude, before a verdict is parsed. Settlement by a scalar the
...
is a outcome-based floor: a judge \emph{with} an energy floor nearly
...
% Deferred detail. In the preprint these blocks are typeset where they were
% written; the venue build moves them here so the page limit costs placement
% rather than evidence. Exactly one build shows each block --
% tests/test_paper_deferred_blocks.py enforces it.
\ifdefined\bodyonly
\section{The strike counter's two shapes}
\label{sec:sweepshapes}
\input{sections/detail/experiments-sweep-shapes}

\section{The persistence attack, counted by provenance}
\label{sec:persistcounters}
\input{sections/detail/experiments-persistence-counters}

\section{The MemoryOS disclosure}
\label{sec:disclosure}
\input{sections/detail/ethics-disclosure}

\section{The CI settlement channel's three writers}
\label{sec:cichannel}
\input{sections/detail/method-ci-channel}

\section{The real-task attack, repository by repository}
\label{sec:attacktransfer}
\input{sections/detail/experiments-attack-transfer}

\section{Reporting rules in full}
\label{sec:reporting-full}
\input{sections/detail/method-reporting}

\section{The regime map's remaining axes}
\label{sec:axes}
\input{sections/detail/regime-axes}

\section{Grids carried as figures and tables}
\label{sec:extrafloats}
\input{sections/detail/figure-adversary}
\input{sections/detail/table-mem0}
\input{sections/detail/table-noise}

\section{Both SWE-Bench-CL matrices}
\label{sec:swebench-result}
\input{sections/detail/experiments-swebench-result}

\subsection{The pilot}
\label{sec:swebench-pilot}
\input{sections/detail/experiments-swebench-pilot}

\section{SWE-Bench-CL protocol}
\label{sec:swebench-protocol}
\input{sections/detail/experiments-swebench-protocol}

\section{Why the attack did not transfer to Mem0}
\label{sec:mem0-analysis}
\input{sections/detail/experiments-mem0-analysis}

\subsection{The phrasing families}
\label{sec:mem0-families}
\input{sections/detail/experiments-mem0-families}

\subsection{What deployed memory systems do when a memory should stop
being used}
\label{sec:deployed}
\input{sections/detail/experiments-deployed-surface}

\section{A note on one survey's two versions}
\label{sec:citationnote}
\input{sections/detail/related-survey-versions}

\section{The neighbouring literature}
\label{sec:neighbours}
\input{sections/detail/related-neighbours}

\section{Threats to validity, in detail}
\label{sec:threats}
\input{sections/detail/limitations-threats}
\fi
