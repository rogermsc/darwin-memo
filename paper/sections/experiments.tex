...
seeds ($+15.3$M, Holm $p = 0.0039$). That result has a sharper form we
did not anticipate. A usage signal is not only a poor proxy for
usefulness; it is a proxy the \emph{attacker} controls. A usage-based
retention variant in our codebase slows an entry's upkeep in proportion
to how often it is recalled and how central it is in an associative
graph, and both of those move on query alone---so a query-only adversary
in the sense of \citet{dong2025minja}, one who never writes and never
settles, drives them without crossing any channel our threat model
names. Because the score is normalised against the live population,
inflating one's own recall count deflates every other entry's: under
flat upkeep the dormant poison starves on exactly the benign schedule
(margin $0$ cycles), and under the usage-based variant with the same
attacker it outlives every benign entry by $4$. The margin is identical
for the loudest payload and the most inert one, because this path reads
usage and never text. We report it as a mechanism demonstration on one
store rather than a rate---it is deterministic and needs no model, but
it is $n=1$---and the system therefore charges flat upkeep by default.
