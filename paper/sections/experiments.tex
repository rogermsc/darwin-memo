...
model apply? --- so we checked what the major deployed agent-memory
systems do when a memory should stop being used. Zep's temporal
knowledge graph does not delete: an outdated fact has its edge marked
invalid and is kept for historical queries. Letta evicts from the
context window and demotes to archival recall rather than removing
anything permanently. Cognee exposes a forget operation whose observed
behaviour leaves the memory retrievable under a forced probe. Mem0 does
delete, but only through the judgment we just measured declining to.

None of them performs \emph{outcome-driven mechanical} deletion, which
is the design a curation-targeted attack needs: a curator that removes
entries, and that decides on a signal rather than on a judgment. Our
