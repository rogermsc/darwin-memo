...
end-to-end \emph{including the real evaluation}: on an Apple-Silicon host
under \texttt{linux/amd64} emulation, a docker-executed instance built the
official SWE-bench image, ran the suite (108/108 pass-to-pass tests
executed), correctly settled an unresolved submission at base behavior,
and wrote schema-valid output, at roughly one minute of wall time per
pytest instance. What remains is not plumbing but a scored run with a
capable model. \todo{Run the docker-evaluated pilot (3 seeds $\times$ 2
sequences, frontier endpoint) and report the per-position resolve-rate and
settlement-delta curves here. This is the single highest-leverage result
still outstanding; the harness is ready.}
