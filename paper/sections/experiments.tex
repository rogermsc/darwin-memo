...
cannot decline to debit. An LLM curator can, and here it did --- it
treated an unsupported retraction as a claim to be recorded rather than
an instruction to be obeyed. So judgment in the curator, which
\S\ref{sec:intro} criticises as expensive and gameable, buys a defence
that mechanical curation does not have. We think both halves are true
and the honest statement is a trade rather than a ranking: a judge costs
a model call per decision and can be talked into things, and it can also
refuse. Our attack is a threat to the mechanical end of that spectrum,
where our own mechanism sits.

Three caveats bound this firmly. It is one system, one curator model,
and one family of attack phrasings; a negative result from a single
probe design is evidence that \emph{this probe} failed, not that the
surface is safe. A weaker or differently-prompted curator may well
delete, and Mem0's DELETE path remains reachable by construction.
