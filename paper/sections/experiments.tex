...
We attempted two, and are precise about what exists, because the released
data contains more than an earlier version of this paragraph claimed. On
\texttt{sympy}, two cells completed in full and are committed:
\texttt{memory\_on} at seeds $0$ and $1$ under attack, $50$
docker-evaluated tasks each. What never completed is what the paired
design needs---their \emph{unattacked} twins, which carry the seeded
poison and therefore cannot be substituted by the long matrix's
\texttt{sympy} cells, plus the third seed and the other two arms. Ten of
the twelve cells a two-sequence result requires are still missing, so no
\texttt{sympy} cell can enter a paired comparison as it stands.
\texttt{sphinx} produced nothing; the container host died a few minutes
into every evaluation. The claim rests on one repository and should be
read as a single-sequence result until that is repaired.
