...
dose from a particular entry, and the first version of this paragraph
said the effect looked ``roughly linear in that level'' on the strength
of them. It is not linear and it is not a level. Running all $32$
...
\paragraph{A correction about how we measured similarity.} The
$0.688$-versus-$0.812$ figures in the previous version of this section
came from a Jaccard we reimplemented in a test helper rather than from
\texttt{MemoryStore.similarity}, the function \texttt{consolidate}
actually calls. The counts it produced were right and its values were
not, and the error was load-bearing: it put the band edges of this sweep
in the wrong places, which is why three of its five registered
predictions are refuted rather than confirmed. The helper now scores
with the retriever, and the pair similarities are pinned by value as
well as by count.
