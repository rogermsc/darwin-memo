...
usefulness; when it is \emph{relative}, it is a proxy the attacker
controls. A usage-based retention variant in our codebase slows an
...
recall term's denominator---the population peak, for a fixed
cap---removes the margin entirely ($4 \rightarrow 0$) while leaving the
starvation horizon unchanged at $\times 1.45$, with graph centrality
still fully attacker-driven throughout that run. So neither usage as
such nor centrality carries the attack. What does is that the score is a
\emph{standing}: one entry's value is a function of every other entry's
traffic, so the adversary takes lifetime from the benign population
rather than buying its own, and no query budget prices it out (one
recall per entry per cycle already buys three of the four cycles). We
report this as a mechanism demonstration on one store rather than a
rate---it is deterministic and needs no model, but it is $n=1$---and the
system charges flat upkeep by default for this reason.
