...
seeds ($+15.3$M, Holm $p = 0.0039$). That result has a sharper form, and it is a second attack rather than a footnote to this one: \S\ref{sec:potentiation}.
Third, \textbf{the honest tie}: on
...
result's value. First, MemoryOS parses several LLM replies with a bare
\texttt{json.loads} and, on failure, silently substitutes a generic
session summary; since retrieval gates on that summary's embedding, a
model that fences its JSON costs the system its topic summaries and makes
the content beneath them unfindable. That is a real fragility in the
...
The objectives have opposite preferred victims
(Table~\ref{tab:persistence}). Destruction destroys the counter and
leaves the ledger standing; persistence leaves the counter untouched and
takes the ledger's poison-kill guarantee away entirely --- $1.00$ to
$0.10$ at two lies per cycle and $0.00$ at four, paired permutation
$p = 0.0039$ on 10 seeds, against \texttt{evict\_on\_negative}'s $1.00$
at every budget ($p = 1$). The persistence attack is also quieter:
benign capability never leaves $1.00$, so nothing moves in the metric an
operator is watching, and only the cumulative delta records the damage.

The explanation is our own central mechanism, seen from the other side.
Bounded credit with earn-back is what lets the ledger forgive a lie it
did not deserve, and it is equally what lets a \emph{paid} poison earn
its way back above the floor: the adversary does not need to prevent
every blame, only to keep the balance positive. The counter cannot
forgive, which is why destruction ruins it --- and cannot be refunded
either, so a single uncovered negative is irreversible and a per-cycle
budget cannot cover every negative indefinitely. Its kill is delayed by
the attack (median cycle $0 \rightarrow 8$ as the budget rises) and never
prevented. Forgiveness is a defence against accidental noise and a
liability against a patient adversary; ruthlessness is exactly the
reverse.

We report this as a correction to our own regime map rather than as a
new win (\S\ref{sec:regime}): on the axis this paper is about, the right
recommendation depends on what the attacker wants, and for a
persistence-seeking attacker the one-line heuristic is the better
choice.
...
own unattacked twin: eighteen cells, six lies fired in every one.
...
\paragraph{What transfers is the mechanism, in its proper currency.}
...
Under the same six lies, the counter's benign burial roughly triples
while the ledger's barely moves, on every seed, with non-overlapping
ranges ($2.29$--$3.00$ against $1.08$--$1.43$). The trigger lists show
the adversary doing both halves of its job: on seed 0 it flipped two
genuine damage signals positive so the guilty went unpunished, and
manufactured six against benign work, and the counter obeyed on every
count. This is the argument of \S\ref{sec:adversary} measured on real
software-engineering tasks: a counter spends a life it cannot recover,
so a lie costs it an entry permanently; a ledger spends energy the entry
earns back, so the same lie costs it almost nothing.

\begin{table}[t]
\centering
\caption{Curation-targeted attack on \texttt{django}, 50 tasks, three
seeds, $b{=}2$ ($17\%$ of settlements corrupted, six lies per cell).
Benign burial is the graveyard minus the poison entries removed. No arm
cleared the poison and no arm lost capability; the separation is in what
the attack made each mechanism delete.}
\label{tab:swebench-attack}
\begin{tabular}{lccc}
\toprule
arm & benign buried, $b{=}0\to2$ & ratio & capability retained \\
\midrule
...
We attempted two (\texttt{sympy}, then \texttt{sphinx}) and neither
completed---the container host died a few minutes into every
evaluation---so the claim rests on one repository and should be read as
a single-sequence result until that is repaired.
