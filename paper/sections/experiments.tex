...
What distinguishes that pair is unexplained. Its Jaccard similarity is
$0.688$ against $0.812$ for the other four, so it is the pair closest to
the $0.55$ merge threshold, and both stores end at $15$ entries --- the
difference is only \emph{which} five merges happened. A
\texttt{merge\_threshold} sweep would separate ``near the threshold''
from ``this content'' and we have not run one. What is settled is the
scope: three findings this paper attributed to a redundant corpus are
one near-duplicate pair in it, and general language was describing a
single entry.
