...
is not a model's opinion. Our starting observation is that coding tasks can produce observable
outcomes, such as a fixed evaluation changing from failure to success.
These outcomes are not physically conserved resources. In our storage
simulation, file sizes are real but the restoration penalty is modeled.
The observation and its delivered report are separate: an adversary can
corrupt the latter without changing the ground-truth record.
Building on the environment-mediated selection principle of
\citet{dodgson2026survival}, we study retention driven by this reported scalar.
...
to it; near-duplicate survivors consolidate, pooling energy. The core retention rule uses no LLM judge. Evaluation and reporting still
have costs, and associating outcomes with consulted entries does not
establish that those entries caused the outcome.
...
It would be easy, and wrong, to claim that outcome-based selection
...
survival driven by reported environmental outcomes, with credit assignment
