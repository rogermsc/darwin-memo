...
quantity: bytes freed, tests passed, issues resolved, spend capped. Where
...
on the one real-task benchmark we ran, where condition (4) failed. The
property that holds across the entire map is that because settlement is a
conserved quantity the environment already produces, there is no proxy to
optimize against, and nothing to hack.
